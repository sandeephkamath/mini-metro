...
The app being backgrounded (the browser tab hidden, or the app switched away from/minimized on a platform that supports that concept) is explicitly *not* a third way to pause the clock. Game time keeps advancing by real elapsed wall-clock time while backgrounded, exactly as if the player had left it foregrounded — on returning, the clock catches up by the full duration that passed, however long that was, with no maximum. This is the opposite of how Background Music behaves (which does pause on backgrounding, per `themes/metro.md` §13) — the two are deliberately not symmetric.

The player may also run the clock at a faster rate than normal (Fast-Forward) rather than only pausing it. Pause, normal speed, and Fast-Forward are three mutually exclusive states the player can select at any time during play — selecting a speed while paused resumes the clock at that speed. This is a normal player control, not a debug/QA-only feature — whether it is actually shown in the HUD is gated by a build-time configuration flag (theme config), hidden by default. While disabled, the clock simply always runs at normal speed and only the Milestone-Event-Choice pause applies; enabling the flag does not change the mechanism described above.
