...
- **Completed**: the player picks one of the two bonus kinds (§2) and it is added to the Reserve. Then, every Node currently in Overflow Risk is relieved in the same way a normal Recovery (core §3 Node Overflow) relieves one: its queue is trimmed back under its maximum capacity — discarding the excess Resources rather than holding or redistributing them, the same "excess is discarded, not returned" rule core §2 Route already uses when a Route is deleted — and its Grace Timer is discarded. The session then resumes from exactly where it left off: same score, same map, same Weeks Survived so far, with the newly-chosen Reserve item ready to place.
