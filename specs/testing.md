...
| Home to Playing | Click Play | Phase goes directly to `playing` — no intermediate instructions overlay; canvas + HUD visible, score 0, week 0 (`themes/home_screen.md`, `themes/metro.md` §8) |
...
| Weekly Upgrade | Advance game time 60s of game time (use debug speed 3x to reach it quickly) | Game pauses, choice popup shows New Train / New Carriage (free, no ad); picking one adds a Depot item, toast shown, game unpauses (`themes/metro.md` §4). Note: as of 2026-07-08 this is a 2-option choice — the former "More Time" kind is removed (`core/monetization.md`); the automated flow under `testing/` still needs updating to match once the code change lands |
...
| Restart | Click the corner close icon on the Game Over screen | Phase returns to `home` with a fresh game state (score 0, week 0); clicking Play again returns directly to `playing` (`themes/home_screen.md`) |
