...
- A "Restart Tutorial" control: a small, standalone icon-only button (same visual weight/style as the other icons), always present regardless of whether this browser has seen the Tutorial before. Starts a brand-new session with the scripted Tutorial (`TUTORIAL.md` §1) forced on, exactly the manual-replay entry point that spec describes — a returning player doesn't have to clear their saved "seen it" state to watch it again.
