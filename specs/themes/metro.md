...
| Cue | Sound | Notes |
|-----|-------|-------|
| Resource Delivered | Two-note ascending chime (E5 → G5) | Passenger delivery — the most frequent cue, kept short and light |
...
**Mute control.** A single mute toggle (speaker icon) silences both Background Music and all Audio Cues together, per core §7 — one control, not independent volume sliders. Present in two places: the Home Screen's top-right icon row (alongside Sign In / Collectibles) and the in-game HUD's top-left corner. The setting is a boolean, persisted to `localStorage` (`src/storage/audioSettings.ts`, same fail-silent posture as `tutorialSeen.ts`), so it carries across sessions.

**Autoplay/gesture handling.** Per core §7's closing note, nothing attempts to play audio before the first user gesture (browsers block it, and Capacitor's WebView on Android inherits the same restriction). A one-time listener for the first pointer/touch/key event anywhere unlocks playback and immediately starts whichever Music Track matches the phase active at that moment — the player never has to interact twice.
