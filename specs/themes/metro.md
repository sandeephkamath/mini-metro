...
| B28 | A debug APK built by GitHub Actions and installed on a real device never showed Play Games identity — no "View Leaderboard" button ever appeared, with no error anywhere | `android-apk.yml` ran `./gradlew assembleDebug` on a fresh runner with no persisted `~/.android/debug.keystore`; Android tooling auto-generates one with a fresh random key pair whenever it's missing, so every CI run produced a debug APK signed with a *different* SHA-1. The Play Games Services Android OAuth client in Play Console is registered against one specific keystore's SHA-1 (the one used when Play Games Sign-In was first set up) — any other signing key makes `GamesSignInClient.isAuthenticated()` legitimately resolve false for this game's project, silently, regardless of whether the signed-in Google account has a valid Play Games profile at the OS level | Added a workflow step that writes that same stable keystore (base64'd into the `ANDROID_DEBUG_KEYSTORE_BASE64` secret, same pattern as the existing `google-services.json` secret) to `~/.android/debug.keystore` before `assembleDebug` runs, so every CI-built debug APK's SHA-1 now matches what's actually registered in Play Console |
