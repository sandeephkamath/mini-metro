...
| Creative Mode Continue enabled | `false` | Whether the Game Over screen's "Continue in Creative Mode" button (core §3 Creative Mode) is shown at all — a build-time/remote-config flag, not a player-facing setting. When `false`, the Game Over screen omits the button entirely and a session that ends stays ended, same as before Creative Mode existed. Remotely switchable via §5.1 like every other config value, without a client release |
...
10. Train rectangles (dark fill, colored border, rotated to direction of travel). A newly-created Train — whether from a Line first connecting two Stations or a Depot Train placement — fades and scales in over the Train spawn animation duration (§5), matching the Station spawn treatment, rather than popping in at full size. A Depot Carriage attaching to an existing Train gets the same fade/scale-in treatment, over the Carriage attach animation duration (§5), scoped to just that carriage's own box (and its coupling link to the carriage ahead of it) — the rest of the Train and its other carriages are unaffected. Each Carriage in a multi-Carriage Train follows the Line's actual track path at a fixed trailing distance behind the one ahead of it (own position and facing angle sampled from the track geometry, not just offset in a straight line from the lead Carriage) — so a linked consist visibly bends through a Line's curves and Station corners the way its coupling links imply, rather than the whole Train moving as one rigid rotated block. The coupling link between two Carriages is drawn between their facing box edges in world space, so it stays taut even when they sit at different angles through a bend.
...
| B29 | On a real Android device, a session that genuinely reached a real Week (confirmed via the Game Over screen's own "Reached Week N" line) submitted to the Leaderboard as exactly `0` instead — reproduced live by declining a "Station Overflow! Watch an ad to continue?" prompt with Play Games Sign-In already active; logcat showed the submission effect firing with `finalWeeksSurvived: 0` several milliseconds *before* the real value was ever computed | `useGameState.ts`'s `declineAdOffer` callback calls `setPhase(...)` immediately after a declined Continue flips `state.phase` to `'gameover'`, for instant UI responsiveness — but the matching `finalWeeksSurvived` value is only computed later, inside the next periodic (~10Hz) `syncReactState()` tick's `sessionEndRecordedRef` block. `GameCanvas.tsx`'s leaderboard-submission effect keyed only on `phase === 'gameover'`, so it fired on the in-between render using `finalWeeksSurvived`'s stale initial value — and since that value defaulted to `0`, indistinguishable from a real session that ends at ~0 weeks survived, nothing caught the staleness. `leaderboardSubmittedRef`'s once-only guard then permanently locked out the correct value once it arrived a render later | `finalWeeksSurvived` now initializes to (and resets to) `null`, not `0` — a sentinel a real survived-weeks value can never collide with. The submission effect now requires `finalWeeksSurvived !== null` in addition to `phase === 'gameover'`, so it can no longer fire on the in-between render regardless of which code path caused the phase transition or how many renders apart the two updates land |
| B30 | The Tutorial's new "Extend the Line" step (`TUTORIAL.md` §5 step 7) rendered its injected Station as a barely-visible, tiny gray blob for the entire step, instead of a normal fully-formed Station | `renderStations.ts`'s spawn-in fade/scale animation is keyed to game time (`state.gameTimeMs - station.spawnedAtMs`), deliberately, so it freezes along with everything else while the Game Clock is held (core §6). The new step spawned its Station via `trySpawnStationAt` at the exact instant it also held the clock (matching `firstLine`/`rescueAct`'s existing paused-while-dragging pattern) — so the Station's age-since-spawn stayed at 0 for the step's entire duration, permanently stuck on the animation's first, smallest, most-transparent frame | The `extendLine` step no longer holds the clock (removed from `CLOCK_HELD_STEPS`, `src/logic/tutorial.ts`) — letting time run lets the spawn animation actually play out over its normal ~600ms, after which the Station sits fully visible while the player drags, same as any Station spawned during real gameplay |
| B31 | The Tutorial's new "Depot Train" step (`TUTORIAL.md` §5 step 10) granted a Reserve Train, but clicking the real Depot button did nothing — no visual feedback, the button never entered its selected state, and the train could never be placed | `GameCanvas.tsx`'s `selectReserveCarrier`/`selectReserveCarriage` unconditionally return early whenever any Tutorial step is active (`tutorialActive`), a deliberate suspension (`TUTORIAL.md` §3) meant to stop a stray click from derailing an earlier, purely-scripted step — but the new Depot Train step's entire point is the player using this exact real control, and the guard blocked it along with everything else | Added a narrow `depotSuspended` check (`tutorialActive && tutorialStep !== 'depotPlace'`) so Depot selection is suspended during every other step as before, but works normally during `depotPlace` — `TUTORIAL.md` §3 now documents the exception explicitly |
| B32 | On a genuinely fresh game, the Tutorial's Welcome and First Line steps (`TUTORIAL.md` §5 steps 1–2) rendered all three starting Stations as barely-visible gray blobs with no discernible shape, instead of normal fully-formed Stations | Same root cause as B30, one layer up: the three starting Stations are created with `spawnedAtMs = state.gameTimeMs`, which is ~0 on a fresh game — and the Tutorial's very first step holds the clock starting at that same ~0, so the Stations' 600ms spawn-in animation never got to run and froze on its first, smallest, most-transparent frame for the entire time the clock stayed held | `startTutorial` (`src/logic/tutorial.ts`) now backdates the three starting Stations' `spawnedAtMs` by `STATION_SPAWN_ANIM_MS` the moment the Tutorial begins, settling their spawn animation instantly — same fix pattern as the Welcome step's Passenger (§5 step 1 note) |
| B33 | The Tutorial's Depot Train step (`TUTORIAL.md` §5 step 5) told the player to "click the train icon at the bottom," but nothing on screen showed where that was — a real player reported not knowing where to click | `renderTutorial.ts`'s `highlightTargets` only ever pulses canvas Stations (`TUTORIAL.md` §4 Station highlight); the Depot Train/Carriage steps' target is a HUD button, which lives in React DOM, not the canvas world space that file draws into — so no highlight mechanism covered this case at all | Added a second, HUD-side highlight: `GameCanvas.tsx` computes `tutorialHighlightDepot` (`'carrier' \| 'carriage' \| null`) from the active step and passes it to `HUD.tsx`, which wraps the matching Depot button in a pulsing orange ring (new `hudDepotHighlightPulse` keyframe), the same visual language as the Station halo — `TUTORIAL.md` §4 now documents this as its own Presentation element |
| B34 | The Tutorial's Extend the Line step (`TUTORIAL.md` §5 step 3) could reclassify a missed handle-grab as "you made a new Line" instead of extending — a real player who dragged from the triangle Station's body instead of its tiny end-tab handle got a new Line and was never shown that Lines could be extended at all. A first fix made the step discard the accidental Line and retry, but that just made the player repeat the same miss behind an apologetic card, and the player asked for that flow to be removed outright | The step relied on the player's drag precisely landing on the end-tab handle (`getLineEndpointAt`, `src/logic/lines.ts`) to distinguish "extend" from "new Line" — the same ambiguity real gameplay's handle-grab always has, just newly exposed by a scripted step that only ever wants one outcome | `onMouseDown` (`src/input/mouseHandler.ts`) now special-cases the `extendLine` step: any mousedown landing near *either* the triangle or star Station captures the drag directly into an extension of the original Line, bypassing handle-precision entirely — `tickTutorial` no longer needs to verify the outcome since it's mechanically guaranteed (`TUTORIAL.md` §5 step 3 detail) |
| B35 | During any Tutorial step that names one specific drag ("drag from the circle station to the triangle station"), the player could instead draw an unrelated Line between two *other* Stations (e.g. square → triangle while the card asked for circle → triangle) — the stray Line didn't satisfy the step and was never explained, so the card just sat there with no visible reason nothing had advanced | Line drawing had no awareness of the Tutorial's current step — any Station could start or extend a Line at any time, same as real gameplay, even while a card was actively instructing a different specific pair | `onMouseDown`/`onMouseMove`/`onMouseUp` (`src/input/mouseHandler.ts`) now consult `tutorialAllowedStations` (`src/logic/tutorial.ts`) on First Line, Extend the Line, and A New Line — a mousedown or mid-drag chain touching any Station outside that step's named pair is ignored outright, so only the instructed drag can ever produce a Line (`TUTORIAL.md` §3) |
| B36 | The square Station sat fully-formed on the board from the instant the Tutorial started, even though nothing in the script looks at it until much later — while the star Station, needed almost immediately (step 3), only appeared once its own spawn-in animation finished. The two felt backwards: the Station used soonest visibly took longer to "arrive" than the one used last | `startTutorial` (`src/logic/tutorial.ts`, B32's fix) backdated all three starting Stations' `spawnedAtMs` uniformly to settle their spawn-in instantly, without regard to which of them the script actually opens with | `startTutorial` now backdates only the circle and triangle Stations — the two First Line actually uses. Left alone, square starts its own natural fade-in at the same moment and finishes during step 2 (which runs the clock), well before it's ever highlighted, instead of being forced complete before the player has even looked at the board |
| B37 | A real player reported the Tutorial stuck on the Depot Train step (`TUTORIAL.md` §5 step 5) — the second Line's Train looked entirely absent, so clicking it to place the new Reserve Train did nothing and the step could never advance | Same root cause as B30, one entity type over: a Train's spawn-in fade/scale is keyed to game time (`renderTrains.ts`). Step 4's new square→triangle Line creates its Train the instant the Line commits, but the very next tick pauses the clock for step 4's Explain card — freezing the Train at its animation's first frame (20% opacity, 40% scale) for the rest of Explain *and* all the way through the paused Depot Train step, since nothing ever resumed the clock in between. At that size and opacity against the map it read as simply not there. `getActiveLineAt` (`src/logic/lines.ts`) also requires a Line to already have a Train before a Reserve one can be placed on it (core §3), so the (invisible but real) Train wasn't actually the blocker — but a player who can't see it has no way to know that | `tickTutorial` (`src/logic/tutorial.ts`) now calls a `settleTrainAnimations` helper every tick the Tutorial holds the clock, backdating any Train's (or attached Carriage's) still-in-progress spawn/attach timestamp so it renders fully settled instead of frozen mid-fade. Checked continuously rather than once on step entry, since a Train can also appear *during* an already-held step (a Depot placement), not just at one's start — `TUTORIAL.md`'s "Note on Trains and held-clock steps" |
...
Not part of `../core/analytics.md`'s Analytics Event primitive (§1-2 there) — a separate, platform-specific capability layered alongside it, same "purely additive, absence changes nothing for the player" posture (core/analytics.md §5).

**Android**: real **Firebase Crashlytics** (`firebase-crashlytics` native SDK + Gradle plugin, same Firebase project as everything else Firebase-backed here), catching native crashes/ANRs automatically once the SDK is present — no app code needed to opt a crash in.

**Web**: no dedicated web crash-reporting product is used. Instead, unhandled JS errors and unhandled promise rejections are routed through the existing Analytics pipeline as a `js_error` event (message + source, §12.1), capped at a small fixed count per session so a pathological repeating error can't flood Analytics. This mirrors AdMob's existing precedent in this document of an Android-native tool with a lighter web fallback rather than a matching web SDK, since Crashlytics itself has no web product.
