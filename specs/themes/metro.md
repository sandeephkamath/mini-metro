...
- The Home Screen (`home_screen.md`) shares this same rotated presentation on a portrait-shaped viewport — it is landscape-shaped like the rest of the game, not a separate portrait-friendly layout. It rotates and resizes using the same computed viewport/rotation as the canvas+HUD, so it fills the physical screen with zero letterboxing whether the device is held in portrait or landscape. Its modal overlays (Collectibles Screen, Leaderboard) inherit the same rotation since they render nested inside it. The Game Over card is a small centered modal, not full-bleed, and is unaffected by this — it stays upright regardless of device orientation.
...
- **Identity**: two paths, an eventual production one and a real interim one.
  - **Production (not yet implemented)**: **Google Play Games Services** supplies the player's identity (display name, avatar), inside the Android-packaged build only.
  - **Interim (implemented now)**: Firebase's own "Sign in with Google" — a browser-compatible popup flow, distinct from Play Games' native Games Sign-In — supplies identity (display name) from whatever Google account the player picks. Available on any platform (web today; Android too, until Play Games Sign-In replaces it there) via the home screen's "Sign In" icon (`home_screen.md` § Leaderboard). There is no separate nickname/account system either way.
- **Backend**: **Firebase** stores and ranks submitted scores. There is no dedicated server, planned or otherwise — the client reads and writes Leaderboard data directly against Firebase, with Firebase's own security rules (not custom server-side logic) as the only gate on what a client is allowed to submit.
...
- **Sign-in**: the interim path is an explicit, player-initiated click — the home screen's "Sign In" icon opens the Google popup described above; declining or closing it leaves the Leaderboard hidden, same as never having clicked it. Once signed in, that identity holds for the rest of the session (not re-prompted, not re-checked). The eventual production path is different in kind, not just source: Play Games sign-in is attempted silently, once, on app launch, using the device's existing Google account, with no prompt or button ever shown — when that ships, it becomes the sole identity source on Android and this interim icon is expected to stop appearing there (still applicable on web, which has no Play Games equivalent).
- **Debug shortcut**: pressing `L` in debug mode (`DEBUG.md` § Debug Leaderboard Sign-In) triggers the same interim popup as the Sign In icon, without needing to click it — purely a testing convenience, not a separate identity path.
