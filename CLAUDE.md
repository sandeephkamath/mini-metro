...
3. **`specs/core/meta_progression.md`** — theme-neutral cross-session progress (Weeks Survived, Best Weeks Survived, the unbounded Collectible Reward sequence, the persistence rules governing all three, and the optional global Leaderboard layered on top). Distinct from `progression.md`: this governs what persists *between* sessions, not in-session difficulty. Read this before changing anything about survival scoring, high scores, collectible/trophy systems, or the leaderboard.
4. **`specs/core/monetization.md`** — theme-neutral ad-gated monetization layered on top of Milestone Events: the on-demand Rewarded-Ad bonus request (unlimited) and the Game-Over Continue (limited per session, ad-gated true continue that rescues a Node Overflow). Milestone Events themselves stay free and unconditional — this document only adds optional extra paths to the same two bonus kinds. Read this before changing anything ad/continue/monetization-related. Not yet implemented in code (spec-only, like Leaderboard/Collectibles below).
5. **`specs/themes/metro.md`** — maps core terms to metro terminology (Station/Line/Train/Passenger), plus all concrete config values (including the progression parameters from `core/progression.md`, the meta-progression parameters from `core/meta_progression.md`, and the monetization parameters from `core/monetization.md`), rendering order, screen states, and the bug log. Read this for anything metro-specific or to find a tunable number.
6. **`specs/themes/home_screen.md`** — the top-level `home` phase shown before a run begins and returned to after game over (distinct from the in-run `start` instructions overlay covered in `metro.md` §8). Best Weeks Survived, the partially-revealed Picture, and a Collectibles Screen entry point (completed + current + a locked preview of upcoming Pictures) are speced for this phase (persistence itself is speced too, per meta_progression.md §6) but not yet implemented in code.
7. **`specs/DEBUG.md`** — debug-mode overlay, spawn/speed controls, manual station/passenger injection. Only relevant when testing or adding debug tooling.
8. **`specs/TUTORIAL.md`** — the scripted first-time-user tutorial (guided first Line, scripted boarding/delivery, can't-fail overflow rescue). Debug-triggered for now (`T` in debug mode); player-facing entry points are deferred to `memo.md` §FTUE.
9. **`specs/testing.md`** — behavior spec for the automated testing agent/harness (flows covered, bug classification rules, report format). Read this before running or extending `testing/`.
10. **`memo.md`** — backlog of undecided/unimplemented future work (styling, scoring, analytics, mobile, persistence, onboarding). Check here before assuming a gap is unintentional, and add to it rather than solving ad hoc if a request expands scope.
11. **`specs/research/`** — not specs. Research notes from watching original Mini Metro playthroughs (frame-by-frame analysis, transcripts, reference screenshots) that informed some of the decisions in `themes/metro.md` and `memo.md`. Cited by path from those docs as supporting evidence; never treat a research note itself as the source of truth for behavior.

Rule of thumb: if a change affects *game rules or entity relationships*, update `core/logic.md`. If it changes *pacing or difficulty* (spawn rates, unlock schedules), update `core/progression.md`. If it changes *cross-session progress* (survival length, high scores, collectibles), update `core/meta_progression.md`. If it changes *ad-gated bonuses or continues*, update `core/monetization.md`. If it's *metro-flavored* (colors, shapes, config numbers, terminology), update `themes/metro.md`. Don't duplicate the same rule across documents — core stays abstract, themes only add mappings/values/visuals.
